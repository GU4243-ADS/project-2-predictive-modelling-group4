...
a<-x[,1:181]
b<-x[,182]
logreg<-glmnet(dat,label_factor, family='binomial')
prediction=predict(logreg,dat,type='class', s= logreg$lambda[length(logreg$lambda)])
...
```{r}
n_test_files <- length(list.files(img_test_dir))


prediction=predict(logreg,dat_test,type='class', s= logreg$lambda[length(logreg$lambda)])
mean(prediction==label_factor)
```
